...
public class MDMatrix<T extends SupportsBinaryOperations<T>> implements Equatable, Invertable, Displayable, SupportsBinaryOperations<Matrix>, SupportsUnaryOperations<Matrix>, SupportsUniversalMultiplication {
    ...
    public enum SupportedUnaryOperations implements SupportedOperationsEnumeration<FailableUnaryOperator<MDMatrix>>{
        ...
        TRANSPOSE("T",(matrix) -> {
            ...
            return Optional.of(new MDMatrix(transposeValues));
        }),
        ...
        MINOR("minor", (matrix) -> {
            ...
            return (Optional.of(new MDMatrix(minorMatrixValues)));
        ...
        private final FailableUnaryOperator<MDMatrix> operator;

        private SupportedUnaryOperations(String symbol, FailableUnaryOperator<MDMatrix> operator){
        ...
        private static Optional<MDMatrix> minorMatrixForLocation(int row, int col, MDMatrix matrix){
            ...
            return Optional.of(new MDMatrix(minorValues));
        }


        private static Optional<MDMatrix> determinantFinder(MDMatrix matrix){
            SupportsBinaryOperations<T> staticObj = new SupportsBinaryOperations<T>();

            if(matrix.getNumberOfColumns() == 2){
                double[][] answer = new double[1][1];
                answer[0][0] = ( staticObj.performBinary(matrix.getValues()[0][0], matrix.getValues()[1][1])) - (matrix.getValues()[0][1] * matrix.getValues()[1][0]);
                return (Optional.of(new MDMatrix(answer)));
            }else{
                ...
                return Optional.of(new MDMatrix(determinantValue));
        ...
        @Override
        public FailableUnaryOperator<MDMatrix> getOperationFunction() {
            return operator;
        }
    ...
    public enum SupportedBinaryOperations implements SupportedOperationsEnumeration<FailableBinaryOperator<MDMatrix>>{
    ...
    public MDMatrix(double[][] twoDArray) throws IllegalArgumentException{
    ...
    public Optional<T[]> getValuesForColumn(int column){
        ...
        for(T[] rowValues : values){
            columnValues.add(counter, rowValues[column]);
            counter += 1;
        }

        T[] arrayOfType = (T[])columnValues.toArray();

        return Optional.of(arrayOfType);
    }
...
//        SupportsUniversalMultiplication newValue;
//
//        if(!(ValueObject.getMyType() instanceof SupportsUniversalMultiplication)){
//
//        }
//
//        for(int i = 0; i < values.length; i++){
//            newValue = (SupportsUniversalMultiplication)(values[i]);
//
//        }
        return;
    }
